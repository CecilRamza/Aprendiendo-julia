...
A=np.zeros((n-1,n-1),dtype=np.float64)  #Matriz con el kernel de las diferencias finitas
B=np.zeros((n-1),dtype=np.float64)      #Vector igualdad de la ecuación diferencial con condiciones de frontera
x=np.linspace(x0,xn,n,dtype=np.float64) #Vector del dominio
